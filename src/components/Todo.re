...
let make = _children => {
...

  {
    ...
    reducer: action =>
      ...
      | RemoveAll => (state => ReasonReact.Update({items: [], inputText: ""}))
      | RemoveCompleted => (
          state =>
            ReasonReact.Update({
              ...state,
              items:
                List.filter(
                  (item: TodoType.item) => item.completed == false,
                  state.items,
                ),
            })
        )
    ...
    render: self => {
      ...
      <div className="app">
        ...
        <button onClick={_ => self.send(RemoveAll)}>
          {ReasonReact.string("Delete all tasks")}
        </button>
        <button onClick={_ => self.send(RemoveCompleted)}>
          {ReasonReact.string("Remove completed tasks")}
        </button>
      </div>;
    },
  };
